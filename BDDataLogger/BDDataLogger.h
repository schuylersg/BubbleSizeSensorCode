...
void DL_Write(uint8_t, uint32_t, uint16_t);	//data type, time(us), value
void DL_Read_Page(uint8_t);		//read the next page if 0, 1 resets to beginning
							//then reads page into buffer and increments read addr
void DL_Flush_All(void);	//force write of partially filled buffer
